# PROJET 1 - Techologie du Web 2 : V'liVe

## Auteurs

#### Alexandre HULSKEN - Zoe CANOEN
#### L2S2 - Gr.2
##### enseignant responsable du module: Bruno Bogaert
##### enseignant de travaux dirigés: Léopold Weinberg

<img src="http://www.fil.univ-lille1.fr/portail/img/logo-FIL-transparent-site.png" width="200"/>

> Ce projet portait sur la création d'une application web permettant aux usagers d'avoir des informations en temps réel des différentes stations V'Lille se trouvant se le territoire, avec la possibilité de faire quelques recherches pour affiner la sélection à travers un formulaire. On y trouve quatres grands axes, une table de l'ensemble des stations, une carte possédant de marqueurs pour chacune d'entre elles, une station sélectionnée et pour finir un formulaire permmetant d'affiner sa recherche d'une station celon certains critères. Les différentes technologies demandé ici ont été: *XHTML5*, *CSS3*, *JS3* et *PHP5*.

> Ce rapport technique permettra de vous expliquer la structure complète de ce projet ainsi que chacun des choix qui ont été fait. Il vous indequera aussi les manques et finira par vous expliquer son utilisation.

---

## Table des matièrentes

1. [L'Arborescence du projet](#arborescence)
2. [L'Explication de nos fichiers](#fichiers)
  1. [Le fichier `index.php`](#index)
  2. [Le dossier lib](#lib)
  3. [Le dossier scripts](#scripts)
  4. [Le dossier style](#style)
3. [Les fonctionnalitées et les manques](#plusEtMoins)
  1. [Les manques de notre projet](#moins)
  2. [Nos différentes fonctionnalités](#plus)
4. [Quelques tests pouvant être effectués](#test)

---

### <a name="arborescence">1. L'Arborescence du projet</a>
...
**Note :** Les codes donnés ici sont considérés comme étant dans le serveur webtp. Si vous les effectez par vous même, vous trouverez des fichiers supplémentaire sans lien direct avec notre application, tel que ce rapport.

### <a name="fichiers">2. L'Explication de nos fichiers</a>

#### <a name="index">2.1. Le fichier `index.php`</a>
...
#### <a name="lib">2.2. Le dossier *lib*</a>
...
#### <a name="scripts">2.3. Le dossier *scripts*</a>
...
#### <a name="style">2.3. Le dossier *style*</a>
...
- L'utilisation du logo de la MEL sans avoir eu la license (nous leur avions envoyé une demande de logo dont nous n'avons eu aucune réponse)
...
Levons le voile, vous pourrez enfin trouver cette application web à l'URL suivante:
`http://webtp.fil.univ-lille1.fr/~hulsken/projet/`
Comme il a été précisé en début de cette fiche, la page principale `index.php` n'apparait pas dans cet URL. Cela se fait que par défaut le navigateur va chercher à lancer un fichier index qui sera la page d'accueil du site hôte demandé (ici `webtp.fil.univ-lille1.fr`) dans le sous-dossier demandé (ici le sous dossier *./~hulsken/projet/*).
Voici donc le test que je vous conseillerais de faire sur cette application:

-> Appuyez sur le bouton nommé *Recherche* puis *Ma station*, vous verrez que l'aide donné ce trouve toujours dans cette section
...
-> Vous verrez que la station en question a été sélectionné et est affiché dans la catégorie *Ma station* (**Note :** si vous étiez dans une autre catégorie que cette-dernière lors de la sélection, vous y êtes redirigé)
...
-> Continuons sur le formulaire, entrez maintenant le nombre `0205,0` dans les deux champs de minimum, vous verrez qu'ils ont été remplacé par `205` ainsi que les champs de maximum puisque leur valeur (qui était `25`) était inférieur
...
**Merci beaucoup à vous pour votre patience**
